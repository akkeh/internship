...
def essDescriptors(dirname, window='hann', M=2048, H=1024, zp=0):
    ...
    for filename in os.listdir(dirname):
        ...
        if filename.find('.json') == -1:
            ...
            filecount += 1
    # add annotated pitches:
    pTag = np.zeros(len(pool['names']), dtype='single')
    ...
    with open('./results/' + dirname.split('/')[-2] + '_ESS.txt') as f:
        ...
        while line != '':
            ...
            for i in range(len(pool['names'])):
                if name == pool['names'][i]:
                    pTag[i] = float(line.split('\t')[4])
...
def calcAll(test=0):
    ...
    for dirname in dirs:
        if pool:
            pool.merge(essDescriptors('./sounds/'+dirname), 'append')
        else:
            pool = essDescriptors('./sounds/'+dirname);
